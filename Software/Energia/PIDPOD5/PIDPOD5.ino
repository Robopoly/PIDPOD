...
void biasCompensation()
{
  ...
  if(mem == NUMBER_SAMPLES)
  {
    ...
    if(bias > 0 && upright_value_accelerometer - UPRIGHT_OFFSET >= UPRIGHT_POSITION - MAX_BIAS)
    {
      upright_value_accelerometer -= UPRIGHT_OFFSET;
    }
    else if(bias < 0 && upright_value_accelerometer + UPRIGHT_OFFSET <= UPRIGHT_POSITION + MAX_BIAS)
    {
      upright_value_accelerometer += UPRIGHT_OFFSET;
    }
   /* Serial.print("Bias: ");
